...
- 將公開推廣頁與會員課程改成兩層視圖：公開層只顯示親子共學、成人跨域與登入；登入後切到會員課程層，不再把會員課程接在首頁下方。
- 調整會員課程順序：三件小行動與拍照保存屬於基礎課程，會排在「解鎖進階 81 格挑戰」之前。
- 拍照上傳後會直接保存到本次共學紀錄，並新增「保存照片與共學紀錄」按鈕。
- 將三件小行動欄位改為帶「範例」提示文字，並同步載入範例內容。
- 將第二個小行動範例改為「週末看一個插畫教學影片」。
- 將載入範例後的三個小行動內容同步加上「範例：」前綴。
- 將三件小行動填寫納入 81 格解鎖條件；行動內容或勾選狀態變更後需重新保存才可解鎖。
- 新增 81 格範例載入功能。
